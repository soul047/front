(() => {
  ...
  // API 키 (환경 변수로 관리)
  const AIRKOREA_KEY = process.env.AIRKOREA_KEY || 'I2wDgBTJutEeubWmNzwVS1jlGSGPvjidKMb5DwhKkjM2MMUst8KGPB2D03mQv8GHu+Rc8+ySKeHrYO6qaS19Sg==';
  const KAKAO_KEY = process.env.KAKAO_KEY || 'df1047fb57dcad7bb8270eae8272c4f6';
  ...
  async function getNearestStation(lat, lon) {
    try {
      // Kakao API로 위/경도 -> TM 좌표
      const tmRes = await fetch(`${KAKAO_COORD_API}?x=${lon}&y=${lat}`, {
      ...
      // 에어코리아 측정소 API
      const res = await fetch(NEARBY_API.replace('{tmX}', tmX).replace('{tmY}', tmY));
